function [hdr, record] = inception_edfread(fname, varargin)
% Read European Data Format file into MATLAB according to INCEPTION rules
% This is a modified version of https://www.mathworks.com/matlabcentral/fileexchange/31900-edfread
%
...
%         workspace.
% [...] = edfread(fname, 'commonFrequency', commonFrequency)
%         If assignToVariables has no value, then 10 is assumed.
%
...
for ii = 1:2:numel(varargin)
    switch lower(varargin{ii})
        ...
        case 'commonsamplerate'
            commonFrequency = varargin{ii+1};
        otherwise
...
if nargout > 1 || assignToVariables
    ...
    record = zeros(numel(hdr.label), commonFrequency*hdr.duration*hdr.records);
    ...
    for ii = 1:hdr.ns
        if ismember(ii,targetSignals)
            ...
            for jj = 1:hdr.records
                try %#ok
                    current_sample_rate = hdr.samples(ii);
                    record(recnum, ctr : ctr + commonFrequency*hdr.duration - 1) = interp1(linspace(1, commonFrequency*hdr.duration, current_sample_rate), tmpdata(jj).data{ii}, 1:commonFrequency*hdr.duration, 'nearest');
                end
                ctr = ctr + commonFrequency*hdr.duration;
            end
            recnum = recnum + 1;
        end
    end
